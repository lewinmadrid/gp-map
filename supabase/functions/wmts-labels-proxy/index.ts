...
serve(async (req) => {
  ...
  try {
    ...
    // Expect path format: /wmts-labels-proxy/{z}/{x}/{y}
    if (pathParts.length < 4 || pathParts[0] !== 'wmts-labels-proxy') {
      console.log('Path parsing debug:', { pathParts, length: pathParts.length })
      return new Response('Invalid path format. Expected: /wmts-labels-proxy/{z}/{x}/{y}', { 
        status: 400,
        headers: corsHeaders
      })
    }
    ...
    const wmtsUrl = `https://geospatialemp.beta.zonehaven.com/geoserver/gwc/service/wmts?` +
      ...
      `SERVICE=WMTS&` +
      `VERSION=1.0.0&` +
      `LAYER=zonehaven:evacuation_zone_ids&` +
      `STYLE=&` +
