/**
 * WMTS Labels Proxy Edge Function
 * 
 * This edge function proxies WMTS label tile requests to your GeoServer/GeoWebCache instance.
 * 
 * Required Supabase Secrets:
 * - WMTS_BASE_URL: Base URL of your WMTS server (e.g., "https://your-geoserver.com/geoserver/gwc/service/wmts")
 * - WMTS_LABELS_LAYER: Labels layer name (e.g., "namespace:evacuation_zone_ids")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Expected path: /wmts-labels-proxy/{z}/{x}/{y}
    const zIndex = pathParts.indexOf('wmts-labels-proxy') + 1;
    if (zIndex === 0 || pathParts.length < zIndex + 3) {
      return new Response('Invalid path. Expected: /wmts-labels-proxy/{z}/{x}/{y}', {
        status: 400,
        headers: corsHeaders,
      });
    }
    ...
    console.info(`Fetching WMTS labels tile: z=${z}, x=${x}, y=${y}`);

    // Get configuration from environment
    const wmtsBaseUrl = Deno.env.get('WMTS_BASE_URL') || 'https://your-geoserver.com/geoserver/gwc/service/wmts';
    const wmtsLabelsLayer = Deno.env.get('WMTS_LABELS_LAYER') || 'namespace:evacuation_zone_ids';
    ...
    wmtsUrl.searchParams.set('LAYER', wmtsLabelsLayer);
    ...
    console.info(`Requesting labels: ${wmtsUrl.toString()}`);
    ...
    console.info(`Successfully fetched labels tile: ${tileData.byteLength} bytes`);
    ...
    console.error('Error fetching WMTS labels tile:', error);
