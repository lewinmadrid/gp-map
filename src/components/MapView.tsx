...
const MapView = () => {
  ...
  useEffect(() => {
    ...
    const cellTowerLayers = [
      "cell-tower-coverage-fill",
      "cell-tower-coverage-outline",
      "cell-tower-points",
      "cell-tower-points-inner",
    ];
    ...
    zoneLayers.forEach((layerId) => {
      ...
      if (layer) {
        map.current?.setLayoutProperty(
          layerId,
          "visibility",
          isNewsMode ? "none" : zoneLayerVisible ? "visible" : "none",
        );
      }
    });
    ...
    return {
      ...
      features: cells.map((cell) => ({
        ...
        properties: {
          ...
          utm: cell.utm,
          bsMc: cell.bsMc,
          rsrp_class: cell.rsrp_class,
          signal_strength:
            cell.rsrp_class === "excellent"
              ? -65
              : cell.rsrp_class === "good"
                ? -85
                : cell.rsrp_class === "fair"
                  ? -100
                  : -110,
        },
  ...
  useEffect(() => {
    ...
    const handleMapClick = (e: any) => {
      ...
      if (selectMode && !measurementMode && !drawingMode) {
        ...
        const drawnPolygons = allFeatures.filter(
          (f) =>
            f.source &&
  ...
  const countPolygonVertices = (feature: any): number => {
    ...
    if (geometry.type === "MultiPolygon") {
      ...
      return geometry.coordinates.reduce((sum: number, polygon: any) => {
        ...
        return (
          sum +
          polygon.reduce((ringSum: number, ring: any) => {
  ...
  const handleDeleteAll = () => {
    ...
    if (style && style.sources) {
      const sourcesToRemove = Object.keys(style.sources).filter(
        (id) =>
          id.includes("drawn-polygon") ||
          id.includes("drawn-circle") ||
          id.includes("uploaded-polygon") ||
      ...
      try {
        ...
        const cellTowerPoints = {
          ...
          features: [
            {
              type: "Feature",
              properties: { cell_id: "SD-001", tech: "LTE", band: "700", azimuth: 120 },
              geometry: { type: "Point", coordinates: [-117.157, 32.729] },
            },
            {
              type: "Feature",
              properties: { cell_id: "SD-002", tech: "5G", band: "850", azimuth: 45 },
              geometry: { type: "Point", coordinates: [-117.117, 32.726] },
            },
            {
              type: "Feature",
              properties: { cell_id: "SD-003", tech: "LTE", band: "1900", azimuth: 270 },
              geometry: { type: "Point", coordinates: [-117.17, 32.705] },
            },
            {
              type: "Feature",
              properties: { cell_id: "SD-004", tech: "5G", band: "700", azimuth: 0 },
              geometry: { type: "Point", coordinates: [-117.192, 32.738] },
            },
          ],
        };
        ...
        map.current.addLayer({
          ...
          paint: {
            "fill-color": [
              ...
              ["get", "rsrp_class"],
              "excellent",
              "#00ff00", // Bright green - excellent signal
              "good",
              "#7fff00", // Yellow-green - good signal
              "fair",
              "#ffff00", // Yellow - fair signal
              "poor",
              "#ff6600", // Orange - poor signal
        ...
        map.current.addLayer({
          ...
          paint: {
            "circle-radius": 10,
            "circle-color": ["match", ["get", "tech"], "LTE", "#2563eb", "5G", "#7c3aed", "#6b7280"],
  ...
  return (
    <div className="relative w-full h-screen bg-background overflow-hidden">
      ...
      {!attributePanelFeature && (selectedPolygons.length > 0 || selectedFeatures.length > 0) && (
              ...
              {selectedPolygons.length > 0 && (
                <div>
                  {selectedPolygons.length === 1 ? (
                    <span>Selected Polygon: {countPolygonVertices(selectedPolygons[0])} vertices</span>
                  ) : (
                    <span>
                      {selectedPolygons.length} Polygons:{" "}
                      {selectedPolygons.reduce((sum, p) => sum + countPolygonVertices(p), 0)} vertices total
                    </span>
                  )}
                </div>
              )}
              {selectedFeatures.length > 0 && (
                <div>
                  {selectedFeatures.length === 1 ? (
                    <span>
                      1 Zone Selected:{" "}
                      {selectedFeatures[0].properties?.zonename_zh?.substring(10) ||
                        selectedFeatures[0].properties?.zone_identifier ||
                        selectedFeatures[0].properties?.id ||
                        "Unknown"}{" "}
                      ({countPolygonVertices(selectedFeatures[0])} vertices)
                    </span>
                  ) : (
                    <span>
                      {selectedFeatures.length} Zones:{" "}
                      {selectedFeatures.reduce((sum, f) => sum + countPolygonVertices(f), 0)} vertices total
                    </span>
                  )}
                </div>
              )}
      ...
      <div className={`absolute top-36 right-4 z-20 flex flex-col gap-1 ${isMobile ? "hidden" : ""}`}>
        ...
        <Button
          variant="secondary"
          size="sm"
          className={`${isMobile ? "w-12 h-12" : "w-10 h-10"} p-0 bg-white/80 hover:bg-white border border-gray-200 shadow-sm transition-colors`}
      ...
      <div
        className={`absolute right-4 z-20 flex flex-col gap-1 ${currentMode === "alert" || currentMode === "news" ? "bottom-20" : "bottom-4"}`}
      >
        {/* Tools Popup Button */}
        <Button
          variant="secondary"
        ...
        >
          {toolsPopupOpen ? (
            <ChevronDown className={`${isMobile ? "h-5 w-5" : "h-4 w-4"} text-gray-600`} />
          ) : (
            <ChevronUp className={`${isMobile ? "h-5 w-5" : "h-4 w-4"} text-gray-600`} />
          )}
        </Button>
      ...
      {currentMode === "alert" && (
        <div className="absolute bottom-4 right-4 z-30 flex gap-2">
          <Button
            variant="outline"
            size="sm"
            className="bg-white/80 hover:bg-white border border-gray-200 text-gray-600 transition-colors"
      ...
      {measurementMode && (
        <div className="absolute top-20 left-20 z-30">
          <div className="bg-blue-100/95 backdrop-blur-sm border border-blue-200 rounded-lg shadow-lg p-3">
            ...
            <div className="text-xs text-blue-600 mt-1">Click on the map to measure distances</div>
            {distances.length > 0 && (
      ...
      {drawingMode && excludeMode && (
        <div className="absolute top-20 left-20 z-30">
          <div className="backdrop-blur-sm border rounded-lg shadow-lg p-3 bg-red-100/95 border-red-200">
            ...
            <div className="text-xs mt-1 text-red-600">Drawing hole in existing polygon</div>
